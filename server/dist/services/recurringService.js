...
export function createRecurring(input) {
    const db = getDb();
    const result = db
        .prepare(`INSERT INTO recurring_transactions
        (type, amount, category_id, description, notes, frequency, start_date, end_date, next_run_date, is_active)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(input.type, input.amount, input.category_id ?? null, input.description ?? null, input.notes ?? null, input.frequency, input.start_date, input.end_date ?? null, input.start_date, // İlk çalıştırma tarihi başlangıçla aynı
    input.is_active === false ? 0 : 1);
    return getRecurring(Number(result.lastInsertRowid));
}
export function listRecurring() {
    return getDb()
        .prepare(`SELECT r.*, c.name as category_name, c.color as category_color, c.icon as category_icon
...
export function getRecurring(id) {
    const row = getDb()
        .prepare(`SELECT r.*, c.name as category_name, c.color as category_color
       FROM recurring_transactions r LEFT JOIN categories c ON c.id = r.category_id
       WHERE r.id = ?`)
        .get(id);
    if (!row)
        throw new HttpError(404, 'Tekrarlayan işlem bulunamadı');
    return row;
}
    ...
    for (const r of items) {
        ...
        db.transaction(() => {
            ...
            while (nextRun <= today) {
                ...
                if (r.end_date && nextRun > r.end_date)
                    break;
                const txRes = db
                    .prepare(`INSERT INTO transactions
              (type, amount, category_id, description, notes, date, recurring_id)
             VALUES (?, ?, ?, ?, ?, ?, ?)`)
                    .run(r.type, r.amount, r.category_id, r.description, r.notes, nextRun, r.id);
                generated++;
