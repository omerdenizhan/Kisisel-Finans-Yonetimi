...
export function createInstallment(input) {
    ...
    return db.transaction(() => {
        const result = db
            .prepare(`INSERT INTO installments
          (name, total_amount, installment_count, installment_amount, start_date, category_id, notes)
         VALUES (?, ?, ?, ?, ?, ?, ?)`)
            .run(input.name, input.total_amount, input.installment_count, base, // installment_amount (görüntü için; ödemeler amounts'unda farklılık olabilir)
        input.start_date, input.category_id ?? null, input.notes ?? null);
        const installmentId = Number(result.lastInsertRowid);
        const insertPayment = db.prepare(`INSERT INTO installment_payments (installment_id, sequence, due_date, amount)
       VALUES (?, ?, ?, ?)`);
...
export function getInstallment(id) {
    ...
    const payments = db
        .prepare(`SELECT * FROM installment_payments WHERE installment_id = ? ORDER BY sequence ASC`)
        .all(id);
    return { ...inst, payments };
}
export function updateInstallment(id, input) {
    const db = getDb();
    ...
    const fields = [];
    const params = [];
    for (const k of ['name', 'total_amount', 'installment_count', 'start_date', 'category_id', 'notes']) {
 ...
 */
export function payInstallment(installmentId, paymentId, date, accountCategoryId) {
    const db = getDb();
    const payment = db
        .prepare('SELECT * FROM installment_payments WHERE id = ? AND installment_id = ?')
        .get(paymentId, installmentId);
    if (!payment)
        throw new HttpError(404, 'Taksit ödemesi bulunamadı');
    if (payment.is_paid)
    ...
    return db.transaction(() => {
        const txRes = db
         ...
         VALUES ('expense', ?, ?, ?, ?, ?, ?)`)
            .run(payment.amount, catId, `${inst.name} - ${payment.sequence}. taksit`, null, paidDate, installmentId);
        const txId = Number(txRes.lastInsertRowid);
        db
            .prepare(`UPDATE installment_payments
...
export function unpayInstallment(installmentId, paymentId) {
    const db = getDb();
    const payment = db
        .prepare('SELECT * FROM installment_payments WHERE id = ? AND installment_id = ?')
        .get(paymentId, installmentId);
    if (!payment)
        throw new HttpError(404, 'Taksit ödemesi bulunamadı');
    if (!payment.is_paid)
        throw new HttpError(400, 'Bu taksit zaten ödenmemiş');
    return db.transaction(() => {
