...
export function getReminder(id) {
    ...
    if (!r)
        throw new HttpError(404, 'Hatırlatıcı bulunamadı');
    return r;
}
export function createReminder(input) {
    const db = getDb();
    const r = db
        .prepare(`INSERT INTO reminders (title, due_date, amount, type, notes)
       VALUES (?, ?, ?, ?, ?)`)
...
export function updateReminder(id, input) {
    const db = getDb();
    getReminder(id);
    const fields = [];
    const params = [];
    for (const k of ['title', 'due_date', 'amount', 'type', 'notes']) {
