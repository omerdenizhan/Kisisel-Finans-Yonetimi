...
export function listTransactions(q) {
    const db = getDb();
    const where = [];
    const params = [];
    if (q.dateFrom) {
  ...
  `;
    ...
    const sums = db.prepare(sumSql).get(...params.slice(0, -2));
    return {
        items: rows,
        total,
        income: sums.income,
...
export function createTransaction(input) {
    ...
    const result = db
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(input.type, input.amount, input.category_id ?? null, input.description ?? null, input.notes ?? null, input.date, input.installment_id ?? null, input.recurring_id ?? null);
