...
import { createInstallment, listInstallments, getInstallment, updateInstallment, deleteInstallment, payInstallment, unpayInstallment, installmentCreateSchema, installmentUpdateSchema, } from '../services/installmentService.js';
...
installmentsRouter.patch('/:id', validate(idParam, 'params'), validate(installmentUpdateSchema), (req, res) => {
    const { id } = validated(req, 'params');
...
installmentsRouter.post('/:id/pay/:paymentId', validate(paymentIdParam, 'params'), validate(payBody), (req, res) => {
    const { id, paymentId } = validated(req, 'params');
    const body = req.body;
    res.json(payInstallment(id, paymentId, body.date, body.account_category_id ?? null));
});
installmentsRouter.post('/:id/unpay/:paymentId', validate(paymentIdParam, 'params'), (req, res) => {
    const { id, paymentId } = validated(req, 'params');
