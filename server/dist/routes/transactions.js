...
import { z } from 'zod';
import { listTransactions, getTransaction, createTransaction, updateTransaction, deleteTransaction, transactionCreateSchema, transactionUpdateSchema, transactionQuerySchema, } from '../services/transactionService.js';
import { validate, validated } from '../middleware/validate.js';
export const transactionsRouter = Router();
const idParam = z.object({ id: z.coerce.number().int().positive() });
transactionsRouter.get('/', validate(transactionQuerySchema, 'query'), (req, res) => {
    res.json(listTransactions(validated(req, 'query')));
});
...
transactionsRouter.patch('/:id', validate(idParam, 'params'), validate(transactionUpdateSchema), (req, res) => {
    const { id } = validated(req, 'params');
