...
import { listCategories, createCategory, updateCategory, deleteCategory, categoryCreateSchema, categoryUpdateSchema, } from '../services/categoryService.js';
...
categoriesRouter.get('/', validate(listQuery, 'query'), (req, res) => {
    const items = listCategories(validated(req, 'query'));
    res.json({ items });
});
categoriesRouter.post('/', validate(categoryCreateSchema), (req, res) => {
    res.status(201).json(createCategory(req.body));
});
categoriesRouter.patch('/:id', validate(idParam, 'params'), validate(categoryUpdateSchema), (req, res) => {
